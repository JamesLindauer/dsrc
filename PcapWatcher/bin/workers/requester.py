...
class Requester():
	...
	def __delete(self):	
		...
		#what if the compression was true
		#the file name would have changed with extention .gz
		#and honestly not gonna bother . delete if exists
		try:
			os.remove("%s.gz" %self.__json_fname)
	...
	def worker_init(self , _queue):
		while True:
			...
			#marked as processsing once the donload is complete
			self.__src_file = re.sub(".inprocessing$" , "" , _abs_file)
			
			#let rename the file as the mod_dsrc looks into file extention
			#so lets remove the ".inprocessing"
			#and set the self.__src_file to what we modified
			try:
				os.rename(_abs_file , self.__src_file)
			except(IOError,OSError) , e:
				self.__log("info" , "Failed While Renaming %s" %(
						 _abs_file ,str(e))
						)
